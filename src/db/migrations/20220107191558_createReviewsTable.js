...
exports.up = function(knex) {
    return knex.schema.createTable("reviews", (table) => {
        table.increments("review_id"); // sets supplier_id as the primary key
        table.text("content");
        table.integer("score");
            table.integer("critic_id").references('critic_id').inTable("critics").onDelete("CASCADE");
        table.integer("movie_id").references('movie_id').inTable("movies").onDelete("CASCADE");
